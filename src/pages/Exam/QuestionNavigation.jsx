...
function QuestionNavigation() {
    ...
    const statusCounts = {
        "not-attempted": Object.values(questionStatus).filter(
            (status) => status === "not-attempted"
        ).length,
        viewed: Object.values(questionStatus).filter(
            (status) => status === "viewed" || status === "marked-review"
        ).length,
        answered: Object.values(questionStatus).filter(
            (status) => status === "answered" || status === "answered-marked-review"
        ).length,
        "marked-review": Object.values(questionStatus).filter(
            (status) => status === "marked-review" || status === "answered-marked-review"
        ).length,
    };
